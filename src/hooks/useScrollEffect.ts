...
export function useScrollEffect({
  ...
  const createThrottledScrollHandlerWithRequestAnimationFrame = useCallback(() => {
      ...
      if (!isAnimationFrameRequested) {
        requestAnimationFrame(() => {
          ...
          if (parallaxElementsContainer?.current) {
            const elementsWithDataSpeedAttribute = parallaxElementsContainer.current.querySelectorAll('[data-speed]');
            elementsWithDataSpeedAttribute.forEach((elementFromQuery) => {
              const htmlElementWithDataSpeed = elementFromQuery as HTMLElement;
              const parallaxSpeedFromDataAttribute = parseFloat(htmlElementWithDataSpeed.dataset.speed || '0.5');
              const parallaxMovementInPixels = currentScrollPositionInPixels * parallaxSpeedFromDataAttribute;
  ...
  useEffect(() => {
    const throttledScrollHandlerFunction = createThrottledScrollHandlerWithRequestAnimationFrame();
    
    window.addEventListener('scroll', throttledScrollHandlerFunction, { passive: true });
